...
const AllUsers = () => {
    ...
    const handleMakeAdmin = user => {
        fetch(`https://bistro-boss-server-galib24.vercel.app/users/admin/${user._id}`, {
            method: 'PATCH'
        })
            .then(res => res.json())
            .then(data => {
                    ...
                    <tbody>
                        {
                            users.map((user, index) =>
                                <tr key={user._id}>
                                    ...
                                    <td>
                                        {
                                            user.role === 'admin' ? 'admin' : <button onClick={() => handleMakeAdmin(user)} className="btn btn-ghost bg-blue-500
